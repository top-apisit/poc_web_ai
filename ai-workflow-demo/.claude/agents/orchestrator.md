...
tools: mcp__jira, mcp__confluence, mcp__figma-remote-mcp, Read, Bash, Agent
...
```bash
# PAGE:
ls app/<name>/page.tsx 2>/dev/null && echo "UPDATE" || echo "NEW"
# COMPONENT:
ls components/<Name>/ 2>/dev/null && echo "UPDATE" || echo "NEW"
# API_ROUTE:
ls app/api/<name>/route.ts 2>/dev/null && echo "UPDATE" || echo "NEW"
```
...
- Cache existing component patterns from components/ui/
