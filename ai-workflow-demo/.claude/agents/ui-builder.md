...
3. **อ่าน existing code patterns (CRITICAL):**
   - `cat components/ui/Button.tsx` → ใช้ existing Button component
   - `cat components/ui/Input.tsx` → ใช้ existing Input component
   - `cat app/globals.css` → ดู global styles และ CSS variables
   - `cat tailwind.config.ts` → ดู theme configuration และ custom classes
4. **ดู project structure:**
   - `ls app/` → ดู route structure (Next.js App Router)
   - `ls components/` → ดู component organization
   - `ls lib/` → ดู utility functions
...
- **Button**: ใช้ `components/ui/Button.tsx` (existing component)
- **Input**: ใช้ `components/ui/Input.tsx` (existing component)
